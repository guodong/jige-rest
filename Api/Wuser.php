        ...
        if ($user) {
        	if("count" == $data["type"]){
        		if(!isset($data["status"])){
	        		...
	        		$ret2 = Db::sql($sql);
	        		$sql = "SELECT COUNT(id) AS count FROM printorder WHERE (status = '已打印' OR status = '已支付')  AND openid = '".$data['openid']."'";
	        		$ret3 = Db::sql($sql);
        		}else{
        			...
        			$ret2 = Db::sql($sql);
        			$sql = "SELECT COUNT(id) AS count FROM printorder WHERE openid = '".$data['openid']."'";
        			$ret3 = Db::sql($sql);
        		}
